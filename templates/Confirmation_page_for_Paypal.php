	<?php
/*
Template name: Confirmation_page_for_Paypal
*/
get_header();
...
// $home_dir = '/home/customer/www/sprint0.com/public_html/sprint0.com';
$home_dir = '/home/customer/www/sprint0.com/public_html';

$Payment_Status = $_GET['Payment_Status'];
		if($Payment_Status == 'SUCCESS'){
	
			$files = file_get_contents( $home_dir . '/paypal/post-data-'. $_GET['cpid'] .'.txt', true);
			//var_dump($files);exit;
			...
			if($files != ''){
				...
				$Res_file = file_get_contents($home_dir . '/paypal/paypal-response-'. $_GET['cpid'] .'.txt', true);
				$Response = json_decode($Res_file,true); 

				$paymentResponse = $Response['state'];
				$cardholder_name = $Response['payer']['payer_info']['first_name'] .' '. $Response['payer']['payer_info']['last_name'] ;

				$CoursePayee_ID = $_GET['cpid'];

				$orderCode = $Response['transactions'][0]['related_resources'][0]['sale']['id'];
			...
			<div class="fusion-row" style="">
				...
				/**************End Mail to bookings@sprint0.com With all attendees Details********************/
							...
							<span style="color: #000000;"> PayPal Payee Name : <?php echo $cardholder_name;?></span><br>
							<span style="color: #000000;"> Amount &nbsp;: <?php echo $Course_balance_due;?></span><br>
							<span style="color: #000000;"> PayPal Transaction ID: <?php echo $orderCode;?><br>
		...
		echo "<center><h1>Invalid Request</h1></center><br>";
			...
			unlink($home_dir . '/paypal/post-data-'. $_GET['cpid'] .'.txt');
			unlink($home_dir . '/paypal/paypal-response-'. $_GET['cpid'] .'.txt');
